...
named!(parse_binary_stl <&[u8], Mesh>, do_parse!(
    ...
    (Mesh { name: "<name unknown>".to_owned(), tris: tris })
));
...
impl Mesh {
    pub fn from_file(fp: &mut File) -> Result<Mesh> {
        ...
        let is_ascii = str::from_utf8(&s[0..5]).map(|header| header == "solid");
        if let Ok(true) = is_ascii {
    ...
    pub fn from_tris(name: &str, tris: Vec<Triangle>) -> Self {
        Mesh {
            name: name.to_owned(),
            tris: tris,
        }
    ...
    pub fn to_text_file(&self, fp: &mut File) -> Result<()> {
        ...
        buf += "end solid";
        fp.write_all(buf.into_bytes().as_slice()).chain_err(
            || "failed to write",
        )?;
        return Ok(());
    }
    ...
    pub fn to_binary_file(&self, fp: &mut File) -> Result<()> {
        ...
        for tri in self.tris.iter() {
            for i in 0..3 {
                buf.write_f32::<LittleEndian>(tri.normal[i]).chain_err(
                    || "format normal",
                )?;
            }
            for i in 0..3 {
                for j in 0..3 {
                    buf.write_f32::<LittleEndian>(tri.verts[i][j]).chain_err(
                        || "format vert",
                    )?;
                }
            }
            if let Some(color) = tri.color {
                ...
                buf.write_u16::<LittleEndian>(clr).chain_err(
                    || "format attr",
                )?;
            } else {
                buf.write_u16::<LittleEndian>(0).chain_err(|| "format attr")?;
            }
        }
        fp.write_all(buf.as_slice()).chain_err(|| "failed to write")?;
        return Ok(());
    }
